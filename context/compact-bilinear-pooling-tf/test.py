...
class BilinearPoolingTest(tf.test.TestCase):
    def testGrad(self):
        with self.test_session():
            p1 = tf.constant([[0.83, 0.50, 0.10, 0.82, 0.24, 0.11, 0.21, 0.51, 0.39, 0.10, 0.67, 0.18]], name="ok")
            p2 = tf.constant([[0.18, 0.59, 0.56, 0.12, 0.33, 0.93, 0.17, 0.58, 0.16]])
            p3 = tf.constant([[0.18, 0.61, 0.56, 0.12, 0.33, 0.93, 0.87, 0.28, 0.16]])

            d = 6
            bp, bp2 = bilinear_pool(p1, p2, p3, d)
            ...
            err = tf.test.compute_gradient_error(p3, [1, 9], bp, [1, 6])
            print('BP Gradient Error (3) : %.2f' % err)
            self.assertTrue(err < 1e-3, 'Gradient must be matched with numerical gradient.')
